      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 h-[31vh] sm:top-14 sm:bottom-auto sm:left-auto sm:right-auto sm:w-[360px] sm:h-[calc(100vh-56px)] bg-[#0f0f0f] border-t sm:border-t-0 sm:border-r border-white/5 rounded-t-[28px] sm:rounded-none overflow-hidden flex flex-col"
            style={{ zIndex: ZINDEX.MAP_CONTROLS }}
          >
            <div className="px-4 pt-3 pb-2 border-b border-white/5 bg-[#101010]">
            ...
            <div className="overflow-y-auto flex-1 p-2">
              {sortedPlaces.map((place) => (
                <button key={place.id} onClick={() => onSelectPlace(place)} className={`w-full text-left p-2.5 rounded-2xl transition-colors mb-2 ${selectedId === place.id ? "bg-red-600/10 border border-red-500/20" : "hover:bg-white/5 border border-white/5"}`}>
                  <div className="flex items-center justify-between gap-3">
                  ...
                  {Number(place.active_hangouts_count || 0) > 0 ? (
                    <div className="mt-1 text-[11px] text-red-400 inline-flex items-center gap-1"><Users className="w-3.5 h-3.5" />{place.active_hangouts_count} plan</div>
                  ) : null}
                </button>
              ))}
            </div>
