...
function StatBox({ label, children }) {
  return (
    <div className="rounded-[16px] border border-black/8 bg-[#fffaf2] p-3">
      <div className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#8a8174]">{label}</div>
      <div className="mt-1 text-[15px] font-black leading-tight text-[#141414]">{children}</div>
    </div>
  );
}
        ...
        <div className="mt-3 flex min-h-0 flex-1 flex-col gap-3 overflow-hidden">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              <h2 className="mt-1 line-clamp-2 text-[clamp(1.7rem,6.4vw,2.2rem)] font-black leading-[0.92] tracking-[-0.06em] text-[#141414]">
                {current.description}
              </h2>
            </div>
          ...
          <div className="rounded-[20px] border border-black/8 bg-[#fffaf2] p-3.5">
            <div className="flex items-start gap-2.5 text-[#605747]">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="line-clamp-2 text-sm leading-6">{current.spot?.address || "NYC"}</div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <StatBox label="Best"> <span className="line-clamp-2">{current.best_slice}</span></StatBox>
          </div>

          <div className="rounded-[20px] border border-black/8 bg-[#fffaf2] p-3.5">
            <div className="flex items-center gap-3">
              <div className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-gradient-to-br from-[#efbf3a] to-[#df5b43] text-sm font-black text-white">
                {avatarLabel(current.host?.username || current.host?.email)}
              </div>
              <div className="min-w-0">
                <div className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#8a8174]">Hosted by</div>
                <div className="truncate text-base font-black text-[#141414]">{current.host?.username || current.host?.email || "Pizza friend"}</div>
              </div>
            </div>
            {current.title ? <p className="mt-3 line-clamp-2 text-sm leading-6 text-[#51493e]">{current.title}</p> : null}
          </div>
        </div>
      </div>
    </motion.div>
...
export default function Descubrir() {
  ...
  const joinMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast({ title: "Te has unido al plan", description: "Ya estás dentro del grupo. Puedes seguir deslizando." });
